...
contract Crowdsale is CrowdsaleBase {
  ...
  /* referral ids
  mapping(address => uint) public referralIds;
  ...
  /**
   * Track who referred the customer so we can send thank you email give them a bonus.
   */
  function investWithReferralId(address addr, uint128 referralId) public payable {
    if(requiredSignedAddress) throw; // Crowdsale allows only server-side signed participants
    if(customerId == 0) throw;  // UUIDv4 sanity check
    referralIds[referralId]+=msg.value
    investInternal(addr, customerId);
  }
   ...
   */
  function getReferralAmounts() public constant returns(FractionalERC20) {
